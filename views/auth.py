"""
POST /auth — получает логин и пароль из Body запроса в виде JSON, далее проверяет соотвествие с данными
 в БД (есть ли такой пользователь, такой ли у него пароль)
...
class AuthView(Resource):
    def post(self):
        ...
        if not (username or password):
            return "Нужен логин и пароль", 400
    ...
    def put(self):
        req_json = request.json
        ref_token = req_json.get("refresh token")
        if not ref_token:
